 ...
 * interoperability. These features are generally heavier-weight ways to interact with the
 * container, so discouraged, but they can be useful to make it easier to use as a drop-in
 * replacement for HashMap. If you don't need them, this class can be preferrable since it doesn't
...
public class SimpleArrayMap<K, V> {
    ...
    /**
     * Caches of small array objects to avoid spamming garbage. The cache Object[] variable is a
     * pointer to a linked list of array objects. The first entry in the array is a pointer to the
     * next array in the list; the second entry is a pointer to the int[] hash code array for it.
     */
    ...
    /**
     * Create a new empty ArrayMap. The default capacity of an array map is 0, and will grow once
     ...
     *
     * <p>
     * This implementation returns false if the object is not a map, or if the maps have different
     * sizes. Otherwise, for each key in this map, values of both maps are compared. If the values
     * for any key are not equal, the method returns false, otherwise it returns true.
     */
    @Override
